...
public:
  //default constructor
  raceDemogData() : FirstNationCount(0), AsianCount(0), BlackCount(0), LatinxCount(0), HIPacificIsleCount(0),
                    MultiRaceCount(0), WhiteCount(0), WhiteNHCount(0), otherCount(0), communityCount(0) {}

  raceDemogData(int FN, int A, int B, int L, int HPI, int MR, int W, int WNH, int size) : FirstNationCount(FN), AsianCount(A), BlackCount(B), LatinxCount(L), HIPacificIsleCount(HPI),
  ...
  void addFirstNationCount(int inNum)
  {
    FirstNationCount += inNum;
    communityCount += inNum;
  }
  void addAsianCount(int inNum)
  {
    AsianCount += inNum;
    communityCount += inNum;
  }
  void addBlackCount(int inNum)
  {
    BlackCount += inNum;
    communityCount += inNum;
  }
  void addLatinxCount(int inNum)
  {
    LatinxCount += inNum;
    communityCount += inNum;
  }
  void addHIPacificIsleCount(int inNum)
  {
    HIPacificIsleCount += inNum;
    communityCount += inNum;
  }
  void addMultiRaceCount(int inNum)
  {
    MultiRaceCount += inNum;
    communityCount += inNum;
  }
  void addWhiteCount(int inNum)
  {
    WhiteCount += inNum;
    communityCount += inNum;
  }
  void addWhiteNHCount(int inNum)
  {
    WhiteNHCount += inNum;
    communityCount += inNum;
  }
  void addOtherCount(int inNum)
  {
    otherCount += inNum;
    communityCount += inNum;
  }
  ...
  {
    FirstNationCount += rd.getFirstNationCount();
    AsianCount += rd.getAsianCount();
    BlackCount += rd.getBlackCount();
    LatinxCount += rd.getLatinxCount();
    HIPacificIsleCount += rd.getHIPacificIsleCount();
    MultiRaceCount += rd.getMultiRaceCount();
    WhiteCount += rd.getWhiteCount();
    WhiteNHCount += rd.getWhiteNHCount();
    otherCount += rd.getOtherCount();
    communityCount += rd.getCommunityCount();
    return *this;
  }
